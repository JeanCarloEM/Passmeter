/**
 * Copyright (c) 2015, Jean Carlo de Elias Moreira. All rights reserved.
 *
 ...
 */
if (typeof (Passmeter) == "undefined" || !Passmeter) {
  (function (root, PM, xlog) {
      ...
      function medir(pass, LANG) {
        ...
        if (_R.length > 0) {
          ...
          if ((PM.cfg.ws.length <= 0) || (!root.key_exists(LANG, PM.cfg.ws))) {
            PM.getDic(LANG);
            xlog('Inexistência de dicionário. Considerando haver palavras na senha.');
    ...
    PM.getLang = function () {
      if (typeof root.window.idioma !== 'string') {
        ...
        xlog('Macro-idioma atual: ' + retorno);
    ...
    PM.getDic = function (langs) {
      ...
      function downDic(url, descri) {
        ...
        root.getJSON(url)(function (json) {
          ...
          xlog("Dicionário [" + descri + "] carregado! ... " + PM.cfg.ws[descri].length + " palavra(s).");
        }, function (status) {
      ...
      if (!PM.cfg.uB.match(/^(ht|f)tps?:\/\/([a-z0-9-\.]+\.[a-z]{2,4}|localhost)\/?([^\s<>\#%"\,\{\}\\|\\\^\[\]`]+)?$/)) {
        throw Error('[DC1] PASSMETER :: URL inválida: [' + PM.cfg.uB + "].");
      }
            ...
            if (scriptsTags !== null) {
              ...
              while ((!achado) && (++index < scriptsTags.length)) {
                if (scriptsTags[index].src.match(XRegExp("(Passmeter(\.min)?\.js(\/?\?.*)?)$", "i")) !== null) {
                  ...
                  achado = true;
                  xlog('URL path: ' + scriptsTags[index].src);
                  xlog('URL base: ' + PM.cfg.uB);
                }
              }
            }
          }
    ...
    global.window.Passmeter.dependencias = function () {
      if (!global.window.Passmeter.isLoaded) {
        if (typeof XRegExp !== "function") {
          ...
          global.loadScript("https://cdnjs.cloudflare.com/ajax/libs/xregexp/2.0.0/xregexp-all-min.js", false, function () {
